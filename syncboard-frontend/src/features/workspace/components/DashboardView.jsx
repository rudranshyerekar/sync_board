  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (wsData.length > 0) {
          const boardsData = await boardApi.getBoards(wsData[0].id);
              ...
              <div>
                ...
                {workspaces.map((ws, wsIndex) => (
                  <div key={ws.id} className="bg-white rounded-xl border border-border shadow-sm overflow-hidden mb-6">
                    <div className="px-6 py-4 border-b border-border bg-gray-50 flex items-center justify-between">
                      ...
                      <span className="text-sm text-gray-500 bg-white px-2 py-1 rounded border border-gray-200">Workspace</span>
                    </div>
                    
                    {wsIndex === 0 && boards.length > 0 ? (
                ...
                {workspaces.length === 0 && (
                  <div className="p-12 text-center border-2 border-dashed border-border rounded-xl">
                    ...
                    {/* Future: Add create workspace button */}
                  </div>
                )}
              </div>
            </>
